...
const apiKey =
	''
const accountId = '5ceb26d8-3f9e-4124-8cfe-62b912d5055b'
...
async function main() {
	...
	const results = await parseCsv('AT-Test-567-holdings.csv')
	...
	for await (let row of results) {
		try
		{
			...
			const price = (parseFloat(row.CostBasis) / parseFloat(row.Shares)).toFixed(4)
